...
app.get('/image', (req: express.Request, res: express.Response): void => {
  ...
  //ImgName def
    ...
    if (fs.existsSync(path.join('assets', `${imgName}.jpg`)) == false) {
      res.send(
        "Error: Invalid input; Image doesn't exist. Please specify image name correctly."
      );
    }
    //ImgName def and existed
      ...
      //Width or Height is not defined
        ...
        res.send(
          'Error: Please specify both height and width via parameters or Just specify name only to show the original image.'
        );
      }
      //Width and height are defined
        ...
        if (
          isValid(parseInt(`${wS}`)) == false ||
          isValid(parseInt(`${hS}`)) == false
        ) {
          res.send(
            'Error: Invalid input; Width and height must be postive integers.'
          );
        }
        //Width and height are valid
          ...
          if (
            fs.existsSync(path.join('thumbs', `${imgName}Thumb${w}_${h}.jpg`))
          ) {
            res.sendFile(
              path.resolve(path.join('thumbs', `${imgName}Thumb${w}_${h}.jpg`))
            );
          }
